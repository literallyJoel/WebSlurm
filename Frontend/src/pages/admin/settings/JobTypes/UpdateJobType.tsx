...
export const UpdateJobType = (): JSX.Element => {
  ...
  const [jobTypeDescription, setDescription] = useState("");
  ...
  useEffect(() => {
    if (currentJob.data) {
      ...
      if (currentJob.data.jobTypeName) {
        setJobTypeName(currentJob.data.jobTypeName);
      }

      if (currentJob.data.jobTypeDescription) {
        setDescription(currentJob.data.jobTypeDescription);
      }

      if (currentJob.data.hasOutputFile) {
        setHasOutputFiles(Number(currentJob.data.hasOutputFile) === 1);
      }

      if (currentJob.data.hasFileUpload) {
        setHasFileUpload(Number(currentJob.data.hasFileUpload) === 1);
      }

      if (currentJob.data.arrayJobSupport) {
        setArrayJobSupport(Number(currentJob.data.arrayJobSupport) === 1);
      }
  ...
  const updateJobTypeRequest = useMutation(
    ...
    (jobType: CreateJobTypeRequest) => {
      return updateJobType(currentJob.data?.jobTypeId ?? "", jobType, token);
    },
    {
      onSuccess: () => {
